...
pub fn load_account_credential(user: &str) -> Result<Option<SecretString>> {
    ...
    {
        let _ = user;
        bail!("the operating-system credential vault is unsupported on this platform")
    }
}
...
fn read_vault_file(path: &PathBuf) -> Result<zeroize::Zeroizing<Vec<u8>>> {
    ...
    if metadata.len() > MAX_VAULT_FILE_BYTES {
        bail!("credential vault exceeds the {MAX_VAULT_FILE_BYTES}-byte limit");
    }
    let raw = zeroize::Zeroizing::new(
        std::fs::read(path).with_context(|| format!("read credential vault {}", path.display()))?,
    );
    if raw.is_empty() {
        bail!("credential vault is empty: {}", path.display());
    }
    Ok(raw)
}
...
fn linux_store(user: &str, password: &SecretString) -> Result<()> {
    linux_secret_service(|connection, service, session| {
        ...
        let collection = zbus::ObjectPath::try_from("/org/freedesktop/secrets/collection/default")
            .context("default Secret Service collection path")?;
        let secret = secret_tuple(&session, password.expose_secret().as_bytes())?;
        let properties = zvariant::Dict::from_iter(
            [
                (
                    "org.freedesktop.Secret.Item.Label",
                    "CentralD OS account credential",
                ),
                ("org.freedesktop.Secret.Item.Attributes", attributes(user)),
            ]
            .map(|(key, value)| (key.to_owned(), zvariant::Value::new(value))),
        );
...
fn linux_search(user: &str) -> Result<Option<String>> {
    ...
    linux_secret_service(|connection, service, session| {
        found = linux_search_inner(&connection, &service, user)?.map(|item_path| {
            let secret: (zbus::ObjectPath, Vec<u8>, Vec<u8>, String) = connection
                .call_method(
                    Some("org.freedesktop.secrets"),
                    &item_path,
                    Some("org.freedesktop.Secret.Item"),
...
fn linux_search_inner(
    ...
    service: &zbus::ObjectPath,
    user: &str,
) -> Result<Option<zbus::ObjectPath>> {
    let attributes = zvariant::Dict::from_iter(
        [("application", "centrald"), ("account", user)]
            .map(|(key, value)| (key.to_owned(), zvariant::Value::new(value))),
    );
    let items: Vec<zbus::ObjectPath> = connection
        .call_method(
            Some("org.freedesktop.secrets"),
            service,
...
#[cfg(target_os = "linux")]
fn attributes(user: &str) -> zvariant::Dict {
    zvariant::Dict::from_iter(
        [("application", "centrald"), ("account", user)]
            .map(|(key, value)| (key.to_owned(), zvariant::Value::new(value))),
    )
}
...
#[cfg(target_os = "linux")]
fn secret_tuple(
    session: &zbus::ObjectPath,
    value: &[u8],
) -> Result<(zbus::ObjectPath, Vec<u8>, Vec<u8>, String)> {
    Ok((
        session.clone(),
        Vec::new(),
        value.to_vec(),
...
fn linux_secret_service(
    ...
    let service = zbus::ObjectPath::try_from("/org/freedesktop/secrets")
        .context("Secret Service object path")?;
    let session: zbus::ObjectPath = connection
        .call_method(
            Some("org.freedesktop.secrets"),
            &service,
            Some("org.freedesktop.secrets.Service"),
...
mod tests {
    ...
    fn secret_tuple_shape_is_stable() {
        ...
        let session = zbus::ObjectPath::try_from("/org/freedesktop/secrets/session/test").unwrap();
