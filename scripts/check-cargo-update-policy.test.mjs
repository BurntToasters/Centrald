...
test('21. no whole-file exemption — scripts/release-helper.js with execSync("cargo update") fails', () => {
  // NOTE: In Centrald, scripts/bump-version.js is an approved automation file that is
  // explicitly excluded. This test uses scripts/release-helper.js to prove that no
  // arbitrary file can receive a whole-file exemption — only exact excluded paths are safe.
  const root = createTempRepo();
  try {
    writeFileSync(
      path.join(root, 'scripts', 'release-helper.js'),
    ...
    assert.equal(ok, false, 'execSync("cargo update") in release-helper.js must be detected');
