...
test("recommended server setup keeps advanced PostgreSQL and custom-config paths explicit", () => {
  const wizard = fs.readFileSync(
    path.join(root, "crates/centrald-server/src/wizard.rs"),
    "utf8",
  );
  ...
  const services = fs.readFileSync(
    path.join(root, "crates/centrald-server/src/services.rs"),
    "utf8",
  );
...
test("managed local PostgreSQL setup is crash-recoverable before first mutation", () => {
  ...
  assert.match(
    recovery,
    /another centrald-server initial-setup process is still provisioning/,
  );
